...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "SELECT p.patient_name, m.prescription_id, m.appointment_id, m.medicine_details, m.dosage, m.duration, m.charge, m.file_path
            FROM prescription m
...
<body>
    <h1>Check Prescriptions provided to OPD patients</h1>
    ...
    <?php if ($searchPerformed): ?>
        ...
        <?php if (!empty($searchResults)): ?>
            <?php foreach ($searchResults as $prescription): ?>
                <div class="prescription-record">
                    ...
                    <?php if (!empty($prescription['file_path'])): ?>
                        ...
                        <a href="<?= htmlspecialchars($prescription['file_path']) ?>" target="_blank">View</a><br>
                    <?php else: ?>
            ...
            <?php endforeach; ?>
        <?php else: ?>
            <p class="error-message">No prescription records found matching your search.</p>
        <?php endif; ?>
    <?php endif; ?>
