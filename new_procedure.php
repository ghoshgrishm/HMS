...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['record_procedure'])) {
        ...
        if ($res && mysqli_num_rows($res) > 0) {
            ...
            $insert = "INSERT INTO procedure_ipd (admission_id, procedure_type, observations, charge)
                       VALUES ('$admission_id', '$procedure_type', '$observations', '$charge')";

            if (mysqli_query($conn, $insert)) {
                $msg = "Procedure added successfully.";
            } else {
                $err = "Error inserting procedure.";
            }
        } else {
...
<div id="procedure-form" class="hidden">
    <form method="post">
        <input type="hidden" name="patient_id" id="patient_id">
        <p><strong>Selected Patient ID:</strong> <span id="pid_display"></span></p>
        Procedure Type: <input type="text" name="procedure_type" required><br>
        Observations: <input type="text" name="observations" required><br>
        Charge in INR: <input type="number" name="charge" step="0.01" required><br>
        <button type="submit" name="record_procedure" class="btn">Record Procedure</button>
    </form>
</div>

<script>
function selectPatient(patient) {
    document.getElementById('procedure-form').classList.remove('hidden');
